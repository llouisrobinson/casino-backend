...
import { CKartTraits } from "../trait/trait.constant";
...
export default class StakingController {
  ...
  public create = async (staking) => {
    try {
      if (
        staking.txType === ETXTYPE.STAKE ||
        staking.txType === ETXTYPE.UNSTAKE
      ) {
      ...
      } else if (staking.txType === ETXTYPE.ADDREWARD_KART) {
        const totalRewardKartAmountTrait = await this.traitService.getItem({
          key: CKartTraits.stakedKartRewardAmount.key,
          name: CKartTraits.stakedKartRewardAmount.name,
        });
        await this.traitService.updateOrInsert(
          {
            key: CKartTraits.stakedKartRewardAmount.key,
          ...
          {
            value: (
              Number(totalRewardKartAmountTrait?.value ?? 0) +
              Number(staking?.amount ?? 0)
            ).toString(),
          }
        );
      } else if (staking.txType === ETXTYPE.ADDREWARD_USK) {
        const totalRewardUskAmountTrait = await this.traitService.getItem({
          key: CKartTraits.stakedUskRewardAmount.key,
          name: CKartTraits.stakedUskRewardAmount.name,
        });
        await this.traitService.updateOrInsert(
          {
            key: CKartTraits.stakedUskRewardAmount.key,
          ...
          {
            value: (
              Number(totalRewardUskAmountTrait?.value ?? 0) +
              Number(staking?.amount ?? 0)
            ).toString(),
          }
        );
      } else {
        const kujiarActionClient = await this.paymentService.getClient();
        const txDetails = await kujiarActionClient.querier.tx.getTx(
          staking.txHash
        );
        const txLowLogs: string = txDetails.txResponse?.rawLog;
        const checkDetails =
          this.paymentService.getTransactionDetails(txLowLogs);
        staking.amount = toHuman(
          BigNumber.from(checkDetails.amount),
          6
        ).toString();
      }
