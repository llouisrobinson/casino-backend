...
import { CKartTraits } from "./trait.constant";
...
export default class TraitController {
  ...
  public getTotalRewardAmount = async () => {
    const totalRewardKartAmountTrait = await this.baseService.getItem({
      key: CKartTraits.stakedKartRewardAmount.key,
      name: CKartTraits.stakedKartRewardAmount.name,
    });
    const totalRewardUskAmountTrait = await this.baseService.getItem({
      key: CKartTraits.stakedUskRewardAmount.key,
      name: CKartTraits.stakedUskRewardAmount.name,
    });
    const totalRewardKartAmount = totalRewardKartAmountTrait?.value ?? 0;
    const totalRewardUskAmount = totalRewardUskAmountTrait?.value ?? 0;
    return {
      value: { usk: totalRewardUskAmount, kart: totalRewardKartAmount },
    };
  };
}
